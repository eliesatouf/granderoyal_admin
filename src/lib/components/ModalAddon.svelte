<script>
  let { showModalAddon = $bindable(),closeModal = $bindable(), header, children } = $props();

  let dialog = $state(); // HTMLDialogElement

  $effect(() => {
    if (showModalAddon) dialog.showModal();
    if(!showModalAddon) dialog.close();
  });
</script>
