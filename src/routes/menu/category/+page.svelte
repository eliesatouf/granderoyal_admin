...
<div class="" in:fade >
	<fieldset class="fieldset bg-base-200 border-base-300 rounded-box  border p-3 m-2 ">
	  <legend class="fieldset-legend">categoryType</legend>
	  <button class="btn btn-primary btn-soft btn-sm w-[100px]"	onclick="{()=>{createcategoryType(), showModal=true}}">	<Icon name="add" />new</button>
		  ...
		  <table class="table table-xs  ">
		    ...
		    <tbody>
		      {#each category as item, i}
		      	<tr><td><button class="btn btn-primary btn-soft btn-sm"	onclick="{()=>{getOnecategoryType(item.id), showModal=true}}">edit	</button>
		      	<td>{i+1}</td>
		      	<td>{item.label}</td>
		    ...
		    <tbody>
		      ...
		      {#each category as item, i}
		      	...
		      	<td><button class="btn btn-primary btn-soft btn-sm"	
		      		onclick="{()=>{getOnecategoryType(item.id,i), showModal=true}}">edit	</button>
		      		</td>
		      	<td>{i+1}</td>
		      	<td>{item.name}</td>
		      	<td>{item.label}</td>
...
<div class="m-2 p-2 rounded md:rounded-lg bg-base-100 max-h-[500px] lg:max-h-[800px] overflow-auto">
  ...
  <div class="flex justify-between">
   <button class="btn btn-soft btn-primary w-1/4" onclick={()=>(saveOnecategoryType())}>
    Save
  </button>
  ...
  <button class="btn btn-soft btn-error w-1/4" onclick={()=>{deletecategoryType(record.id), showModal = false}}>
    Delete
  </button>
...
async function getCategories(){
  const response = await useFetch('/categories', 'GET',null, true);
  category = response

}

async function getOnecategoryType(id){
	modalOperation= 'Edit'
    const categoryData =  await useFetch( '/categories/'+id,'GET',null,true);
    record={...categoryData}
    modalHeader = record.name
    modalOperation="Edit categoryType"

}

async function saveOnecategoryType(index){
	//console.log(record)
	let saveItem=''
	if(modalOperation=='Create category'){
		try{
			saveItem =  await useFetch('/categories', 'POST',record, true);
			if(saveItem.status && saveItem.status != 200){
				toast.error("Failed to create category",2000);
			}else{
		...
		try{
  		let saveItem =  await useFetch('/categories/' + record.id, 'PATCH',record, true);
...
async function handleSelectcategoryType(item){
	const index = category.findIndex(item=>item.id == record.id)
...
async function deletecategoryType(id){
  try{
  	let saveItem =  await useFetch('/categories/' + id, 'DELETE',null, true);
  	toast.success("Deleted successfully",2000);	
  	getCategories()
  }
	catch(e) {
  	toast.error("Failed to delete category",2000);	
  	console.log(e);
  }
}
...
async function goToSubcategoryType(subcategoryType){

	userState.selectedSubcategoryType=subcategoryType
	goto('/category/subcategoryType', { replaceState: true });
}
