      ...
      <div class="h-32 bg-base-300 ">
		    <div class="h-32 overflow-y-auto ">
		     	{#each record.bundleItems  as dish,index}
		     	    ...
		     	    <div class="text-info">
		     			{dishList.find(el=> el.id === parseInt(dish.dish.split('/')[3]))?.promoStatus.description ?? 'na'}
		     		</div>
		     	    <div class=" basis-1/2">
...
async function getDishes(){
  const response = await useFetch('/dishes?active=true&type=dining&hasVariant=false', 'GET',null, true);
  dishList = response
}
