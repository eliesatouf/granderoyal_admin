...
export const actions = {
  default: async ({ url, request, fetch }) => {
    ...
    try {
      ...
      const response = await fetch('https://www.granderoyalerestaurante.com/api/loyverse/token', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        throw redirect(303, '/admin/loyverse-test?success=true&items=' + data.items_fetched);
      } else {
        // 5. On backend error, redirect with an error message
        throw redirect(303, '/admin/loyverse-test?error=' + encodeURIComponent(data.error || 'Token exchange failed'));
      }
    } catch (err) {
      // 6. On network error, redirect with error details
      throw redirect(303, '/admin/loyverse-test?error=' + encodeURIComponent(err.message || 'Network error'));
    }
  }
};