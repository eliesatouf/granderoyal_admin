<div  class="grid grid-col lg:flex lg:flex-wrap justify-center  ">
	{#if isLoading}
    ...
    <div class="grid bg-base-300 border-base-300 rounded-box w-[250px] lg:w-[280px]  lg:p-1  border p-1">
      <fieldset class="fieldset bg-base-200 border-base-300 rounded-box  w-[230px] p-1">
        ...
        <div class="grid">
          <label class="input">
            <span class="">Was</span>
            <input type="number" class="overflow-hidden" placeholder="price was" 
             bind:value="{record.priceWas}"/>
          </label>
          
          <label class="input">
            <span class="">Now</span>
            <input type="number" class="overflow-hidden" placeholder="price now" 
             bind:value="{record.priceNow}"/>
          </label>
        </div>
      </fieldset>

     <fieldset class="fieldset bg-base-200 border-base-300 rounded-box lg:p-1  border p-1  lg:w-[230px]">
		    ...
		    <div class="h-32 overflow-y-auto p-3">
		    	<button class="btn btn-sm btn-soft btn-primary" onclick="{()=>{addingDishTo ='bogoDishes', showModalRecord=true}}">BOGO Dish
		     	...
		     	{#each record.bogoEligibleDishes  as dish,index}
		     		...
		     		<div class="text-info">
		     			{dishes.find(el=> el.id === parseInt(dish.split('/')[3]))?.promoStatus.description ?? 'na'}
		     		</div>
		     	{/each}
		   		</div>
	    	</div>
...
<div class="h-56 overflow-auto">
	...
	{#each searchResults as dish}
		...
		<badge class="badge {dish.id ==getDiscountDishId(dish.id)? 'badge-success':'badge-default'}"
			>{dish.type} {dish.name} 
	 </badge>
	 <badge 
		class="text-info  text-wrap"
		> {dish.promoStatus.description}
	 </badge>
...
async function getSelectedBudleDishes(){
	...
	record.priceWas = bundle.originalPrice
	record.priceNow = parseInt(bundle.price)
	isLoading=false
}
