<!-- src/routes/reports/weekly/+page.svelte -->
<script>
  ...
  async function loadWeeklyReport(date) {
    ...
    try {
      ...
      const data =  await useFetch(`/dining/sales/reports/weekly/${date}`,'GET',null,true);
      reportData = data;
...
<div class="min-h-screen bg-gray-100 p-6">
  <div class="max-w-7xl mx-auto">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900">Weekly Sales Report</h1>
      
      <div class="flex items-center gap-4 mt-4">
        <label class="text-sm font-medium text-gray-700">Select Date in Week:</label>
        <input 
          type="date" 
    ...
    {:else if error}
      <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
        ...
        <button on:click={() => loadWeeklyReport(selectedDate)} class="mt-3 bg-red-600 text-white px-4 py-2 rounded-md text-sm hover:bg-red-700">
          Retry
        </button>
      </div>
    {:else if reportData}
      ...
      <div class="bg-white rounded-lg shadow p-6 mb-8">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Top Selling Items This Week</h3>
